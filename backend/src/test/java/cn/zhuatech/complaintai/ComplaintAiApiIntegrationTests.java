 ...
 */
@SpringBootTest @AutoConfigureMockMvc class ComplaintAiApiIntegrationTests { @Autowired MockMvc mvc; private String token;
 /**
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
  ...
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
 @Test void operatorCanAnalyzeComplaint()throws Exception{mvc.perform(post("/api/ai/complaint/analyze").header("Authorization","Bearer "+token).contentType(MediaType.APPLICATION_JSON).content("{\"complaintNo\":\"CP-88\",\"channel\":\"热线\",\"sentimentScore\":20,\"repeatComplaints\":3,\"overdueMinutes\":280,\"safetyOrComplianceRisk\":true,\"compensationAmount\":18000,\"socialMediaExposure\":true}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.severity").value("CRITICAL"));}}
